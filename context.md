...
- **FIREBASE ADMIN SDK PRIVATE KEY & TOKEN AUTH FIX**: Fixed [backend/src/config/firebase.ts](file:///g:/TSS/backend/src/config/firebase.ts) RSA private key unescaping (`serviceAccount.private_key.replace(/\\n/g, '\n')`) and added a lazy Firestore Proxy so top-level collection imports never crash Node.js startup on Render. Fixed [backend/src/middleware/auth.ts](file:///g:/TSS/backend/src/middleware/auth.ts) `verifyIdToken` verification flow for Google Sign-In and email logins.
- **GITHUB PAGES CNAME & DEPLOYMENT FIX**: Fixed [public/CNAME](file:///g:/TSS/public/CNAME) to `tsscomputer.in` (was set to wrong domain `tsscomputerstore.com` causing GitHub Pages 404). Updated [.github/workflows/deploy.yml](file:///g:/TSS/.github/workflows/deploy.yml) default `VITE_API_URL` to `https://the-tss-computer-store.onrender.com/api`.
