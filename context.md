...
## Changes Made
- **REORDERED HERO PRODUCTS**: Updated `heroProducts` in [src/pages/Home.tsx](file:///g:/TSS/src/pages/Home.tsx) so `TSS Blade X1 Gaming Laptop` (`heroLaptopOne`) is the 1st hero product and `TSS Studio Pro Workstation` (`heroLaptopTwo`) is the 2nd.
- **#HOME ANCHOR ROUTING & INTERNAL REDIRECTS**: Added `id="home"` anchor container and hash scroll handler to [src/pages/Home.tsx](file:///g:/TSS/src/pages/Home.tsx). Updated [src/components/StaggeredMenu.tsx](file:///g:/TSS/src/components/StaggeredMenu.tsx) and [src/components/Layout.tsx](file:///g:/TSS/src/components/Layout.tsx) to route TSS logo and Home navigation links to `/#home` instead of external domain reloads, skipping intro scroll sequences and smoothly scrolling into `#home`.
- **SWAPPED 1ST AND 2ND HERO PRODUCTS**: Swapped the positions of the 1st and 2nd products in `heroProducts` array inside [src/pages/Home.tsx](file:///g:/TSS/src/pages/Home.tsx) so that `TSS Studio Pro Workstation` (`heroLaptopTwo`) is now the default 1st hero product and `TSS Blade X1 Gaming Laptop` (`heroLaptopOne`) is 2nd.
- **STRICT SCROLL ENTRY PAGE HEADER CONTROLS HIDING**: Added active `isScrollEntryPage` check in [src/components/StaggeredMenu.tsx](file:///g:/TSS/src/components/StaggeredMenu.tsx) to guarantee that all right-side header controls (search icon, language switcher pill `EN|हिन्दी`, and `Menu +` button) are 100% hidden on the starting scroll entry page, displaying strictly the **TSS** logo. Clicking the TSS logo instantly dispatches `introComplete`, unmounts the scroll entry page, resets scroll position to `(0, 0)`, and reveals the main home page with all controls.
- **INSTANT SCROLL SKIP & DIRECT HOME TOP REDIRECT**: Updated [src/pages/Home.tsx](file:///g:/TSS/src/pages/Home.tsx), [src/components/IntroScroll.tsx](file:///g:/TSS/src/components/IntroScroll.tsx), [src/components/StaggeredMenu.tsx](file:///g:/TSS/src/components/StaggeredMenu.tsx), and [src/components/Layout.tsx](file:///g:/TSS/src/components/Layout.tsx) to ensure clicking the TSS logo on the scroll screen or from any page instantly kills all GSAP ScrollTriggers, unmounts the scroll sequence, resets scroll position immediately (`window.scrollTo(0, 0)`), and lands directly on top of the main home store page.
- **SOLO TSS LOGO ON SCROLL SCREEN**: Added `hideControls` prop to [src/components/StaggeredMenu.tsx](file:///g:/TSS/src/components/StaggeredMenu.tsx) and updated [src/components/Layout.tsx](file:///g:/TSS/src/components/Layout.tsx) to hide the search button, language switcher, and menu toggle during the intro scroll sequence, displaying strictly the **TSS** logo on the left. All controls smoothly reappear once the intro scroll is completed or skipped by clicking the TSS logo.
- **REMOVED OVERLAPPING INTRO SCROLL LABELS**: Removed the duplicate `hud-top-nav` labels (`TSS HOME` and `SKIP SCROLL ➔`) from [src/components/IntroScroll.tsx](file:///g:/TSS/src/components/IntroScroll.tsx) to eliminate UI visual overlap with the main navigation bar.
- **MOBILE NUMBER FIELD & FIREBASE MESSAGES INTEGRATION**: Added a mandatory Mobile / Phone Number input field (`phone`) across all contact forms ([src/pages/Contact.tsx](file:///g:/TSS/src/pages/Contact.tsx) and footer form in [src/components/Layout.tsx](file:///g:/TSS/src/components/Layout.tsx)). Updated backend API route [backend/src/routes/messages.ts](file:///g:/TSS/backend/src/routes/messages.ts) and model [backend/src/models/Message.ts](file:///g:/TSS/backend/src/models/Message.ts) to store phone numbers in the Firebase `messages` Firestore collection. Updated CMS Messages tab in [src/pages/AdminDashboard.tsx](file:///g:/TSS/src/pages/AdminDashboard.tsx) to fetch and render customer mobile numbers in both the real-time table and message detail view modals.
- **HOME LOGO & SKIP SCROLL IN NAV ON SCROLL SCREEN**: Added a prominent **TSS HOME** logo button and a **SKIP SCROLL ➔** pill button to the navigation bar overlay in [src/components/IntroScroll.tsx](file:///g:/TSS/src/components/IntroScroll.tsx). Updated [src/components/Layout.tsx](file:///g:/TSS/src/components/Layout.tsx) and [src/components/StaggeredMenu.tsx](file:///g:/TSS/src/components/StaggeredMenu.tsx) so the main header navigation is always accessible during the intro scroll screen, and clicking the TSS Home logo or Home menu link immediately skips the 350% 60-frame canvas scroll sequence and smoothly transitions directly to the top of the main store.
...
- **FIREBASE ADMIN SDK PRIVATE KEY & TOKEN AUTH FIX**: Fixed [backend/src/config/firebase.ts](file:///g:/TSS/backend/src/config/firebase.ts) RSA private key unescaping (`serviceAccount.private_key.replace(/\\n/g, '\n')`). Added robust `try...catch` fallback initialization to prevent complete `Exited with status 1` Node.js server startup crashes when the `FIREBASE_SERVICE_ACCOUNT_BASE64` env string is corrupted or contains invalid JSON/Base64 characters. Added a lazy Firestore Proxy so top-level collection imports safely resolve. Fixed [backend/src/middleware/auth.ts](file:///g:/TSS/backend/src/middleware/auth.ts) `verifyIdToken` verification flow for Google Sign-In and email logins.
- **GITHUB PAGES CNAME & DEPLOYMENT FIX**: Fixed [public/CNAME](file:///g:/TSS/public/CNAME) to `tsscomputer.in`. Updated [public/404.html](file:///g:/TSS/public/404.html) (`pathSegmentsToKeep = 0`) and [index.html](file:///g:/TSS/index.html) (URL decoder snippet) to fix the `~and~` infinite redirect loop when refreshing pages on a custom domain. Updated [.github/workflows/deploy.yml](file:///g:/TSS/.github/workflows/deploy.yml) default `VITE_API_URL` to `https://the-tss-computer-store.onrender.com/api`.
- **RENDER KEEP-ALIVE CRON WORKFLOW**: Created [.github/workflows/keep_alive.yml](file:///g:/TSS/.github/workflows/keep_alive.yml) to automatically ping `https://the-tss-computer-store.onrender.com/api/health` every 14 minutes to bypass Render free tier cold starts and keep the backend active 24/7.
- **API ROUTING & RENDER INTEGRATION**: Updated [src/lib/api.ts](file:///g:/TSS/src/lib/api.ts) to prioritize `VITE_API_URL` environment variable and fallback seamlessly to the Render deployment endpoint (`https://the-tss-computer-store.onrender.com/api`).
