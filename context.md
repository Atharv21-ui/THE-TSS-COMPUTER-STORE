...
- **FIREBASE ADMIN SDK BASE64 & AUTH FIX**: Updated [backend/src/config/firebase.ts](file:///g:/TSS/backend/src/config/firebase.ts) with whitespace-resilient Base64 and raw JSON parsing to prevent Render deployment crashes when pasting `FIREBASE_SERVICE_ACCOUNT_BASE64`. Enhanced [backend/src/middleware/auth.ts](file:///g:/TSS/backend/src/middleware/auth.ts) with detailed error diagnostics.
